...
export default function Home() {
  ...
  const handleExport = useCallback(async () => {
    ...
    setExporting(true);
    try { await exportPreview({ frameEl: previewRef.current, username: profile.username }); }
    catch { alert("Não foi possível exportar. Tente novamente."); }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="relative flex-1 overflow-auto bg-slate-100 dark:bg-[#0a0a0a] flex flex-col items-center py-6 px-4 gap-5">
          ...
          <div className="relative flex items-center gap-2 flex-wrap z-10">
              ...
              {(["mobile","desktop"] as DeviceView[]).map(mode => (
                <button key={mode} onClick={() => setDeviceView(mode)}
                  className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold transition-all ${
                    deviceView === mode ? "ig-gradient text-white" : "text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
                  }`}>
                  {mode === "mobile"
            ...
            <div className="w-px h-6 bg-slate-200 dark:bg-slate-800 hidden lg:block" />
            <span className="text-[11px] text-slate-400 dark:text-slate-600 hidden lg:block">
          ...
          <a href="https://www.guebly.com.br" target="_blank" rel="noopener noreferrer"
            className="relative z-10 flex items-center gap-2 text-[11px] text-slate-400 dark:text-slate-600
              hover:text-slate-600 dark:hover:text-slate-400 transition pb-4">
