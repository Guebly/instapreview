...
export default function PhonePreview({ profile, highlights, feed, onFeedReorder, igTheme }: Props) {
  ...
  return (
    <div
      className="phone-shadow w-[375px] flex-shrink-0 rounded-[44px] border-[8px] border-[#2a2a2a] overflow-hidden"
      style={{ background: c.bg, outline: "1px solid #555" }}
    >
      {/* Notch */}
      <div className="relative" style={{ background: c.navBg }}>
        ...
        <div className="pt-8 ig-font">
          ...
          <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: c.border }}>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {profile.verified && (
                <svg width="14" height="14" viewBox="0 0 24 24" fill="#3797f0">
          ...
          <div className="px-4 pt-4">
            ...
            <div className="mb-3">
              <div className="flex items-center gap-1.5 mb-0.5">
                ...
                {profile.verified && (
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="#3797f0">
                    <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" />
                  </svg>
                )}
              </div>
              {profile.category && (
                <p className="text-[13px] font-normal" style={{ color: c.textSec }}>{profile.category}</p>
              )}
              {profile.bio && (
                <p className="text-[13px] whitespace-pre-line leading-[1.45] mt-0.5" style={{ color: c.text }}>{profile.bio}</p>
              )}
              {profile.link && (
            ...
            <div className="flex gap-1.5 mb-3">
              ...
              {profile.ctaLabel ? (
                ...
                <button className="py-[6px] px-2.5 text-[13px] font-semibold rounded-lg border"
                  style={{ background: c.btnBg, borderColor: c.border, color: c.text }}>
          ...
          {/* Highlights */}
          {highlights.length > 0 && (
            <div className="flex gap-4 px-4 pb-4 overflow-x-auto border-b" style={{ borderColor: c.border, scrollbarWidth: "none" }}>
              {highlights.map(h => (
                <div key={h.id} className="flex flex-col items-center gap-1 flex-shrink-0">
                  <div className="w-[62px] h-[62px] rounded-full border p-[2px]" style={{ borderColor: c.hlBorder, background: c.bg }}>
                  ...
                  <span className="text-[11px] max-w-[62px] text-center overflow-hidden text-ellipsis whitespace-nowrap" style={{ color: c.text }}>
                    {h.name || "Destaque"}
                  </span>
                </div>
              ))}
